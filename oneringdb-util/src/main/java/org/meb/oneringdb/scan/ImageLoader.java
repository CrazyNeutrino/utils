...
public class ImageLoader {
	...
	private static void loadCardImagesFromCardgamedb() {
		...
		// String cslSymbol =
		// csb.getLangItems().get("en").getSymbol().toLowerCase();
		// if (cslSymbol.equals("cs")) {
		// cslSymbol = "core";
		// }
		// String path = System.getProperty("dropbox.home")
		// + "/workspace/lotrlcg/images/cards/med_" + cbTechName + "-" +
		// cslSymbol
		// + ".jpg";
		// if (!new File(path).exists()) {
		// urls.add(CARD_URL_BASE.replace("${name}",
		// cbTechName).replace("${set}", cslSymbol));
